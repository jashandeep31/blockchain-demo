...
io.on("connection", (socket) => {
    ...
    socket.on("myblockchain", ({ id, blockchain, publicKey, }) => {
...
const firstTransaction = () => {
    ...
    const baseTransaction = {
        amount: 100,
        from: Keypair.fromSecretKey(bs58.decode(process.env.SECRET_KEY || "")).publicKey.toBase58(),
        to: secAccount.publicKey,
        timestamp: Date.now(),
    };
    const transaction = {
        ...baseTransaction,
        signature: bs58.encode(nacl.sign.detached(decodeUTF8(JSON.stringify(baseTransaction)), bs58.decode(process.env.SECRET_KEY || ""))),
    };
...
app.post("/api/v1/send", async (req, res) => {
    ...
    const checkTransaction = async (signature) => {
        let response;
        do {
            response = await axios.get(`http://localhost:3000/transaction?signature=${signature}`);
            if (response.data) {
                return response;
            }
...
app.get("/api/v1/balance", async (req, res) => {
    ...
    const minerResponse = await axios(`http://localhost:3000/balance?key=${publicKey}`);
    return res.status(200).json({
...
app.get("/api/v1/transactions", async (req, res) => {
    ...
    const minerResponse = await axios(`http://localhost:3000/transactions?publicKey=${publicKey}`);
    return res.status(200).json({
